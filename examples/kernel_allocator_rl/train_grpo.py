...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a GRPO-style policy for the RL allocator.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        print(
            "grpo dry-run training using trace: "
            f"{trace_path} ({len(trace)} events, epochs={args.epochs})",
        )
        return 0

    raise SystemExit(
        "Full GRPO training is not exercised in this environment. "
